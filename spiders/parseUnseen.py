...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print(response.url)
        metadata = dict()
        savepath_Clfeval = 'unseen.csv'
        eval_metrics = dict()

        metadata['url'] = response.url

        eval_metrics['url_chars'] = len(response.url)
        p, lns = Counter(response.url), float(len(response.url)) 
        eval_metrics['entropy']= -sum ( count/lns * math.log(count/lns, 2) for count in p.values())
        ...
        # Save as CSV
        with open(savepath_Clfeval, 'wb') as f:
            print('Saving the output in  csv format!')
            # Add header to beginning of file
            f.write('phishing,average_link_len,'
                    'num_shortened_links,num_redirect_links,num_https_links,num_images,num_external_images,tagret_url_len,favicon_match,target_url_https,trademark_in_target_url,'
                    'days_since_creation,days_since_update,days_until_expiration,number_of_links,entropy\n')
            ...
            f.write(str(eval_metrics['entropy']) + '\n')
